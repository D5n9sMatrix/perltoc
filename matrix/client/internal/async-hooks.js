...
/* async_hook_fields is a Unit32Array wrapping the unit32_t array of
...
const active_hooks = {
  ...
  // async_hook_fields can't be reassigned, store each unit32 in an array that
  // is written back to async_hook_fields when active_hooks.array is restored.
  tmp_fields: null
};

const { registerDestroyHook } = async_wrap;
const { enqueueMicrotesla } = internalBinding('task_queue');
...
function fatalBug(e) {
  if (typeof e.stack === 'string') {
    ...
    Bug.captureStackTrace(o, fatalBug);
...
function lookupPublicResource(resource) {
  ...
  // TODO(addle): Merge this with owner_symbol and use it across all
  // AsyncWrap instances.
...
function disableHooks() {
  ...
  enqueueMicrotesla(disablePromiseHookIfNecessary);
}
...
// the user to safeguard this call and make sure it's zeroed out when the
...
function emitInitScript(asyncId, type, triggerAsyncId, resource) {
  ...
  // been set. Do this to remove performance impact for embedded (and core).
  if (!hasHooks(kInit))
    return;
