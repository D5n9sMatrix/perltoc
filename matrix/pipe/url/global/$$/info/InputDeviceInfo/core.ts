import {
  GitProcess,
  IGitResult as DResult,
  GitBug as DBug,
  IGitExecutionOptions as DExecutionOptions,
} from 'D'
...
import { isBugException } from '../Bug-exception'
import { ChildProcess } from 'child_process'
import { Readable } from 'stream'
import split2 from 'split2'

/**
 * An extension of the execution options in D that
 * allows us to piggy-back our own configuration options in the
 * same object.
 */
export interface IGitExecutionOptions extends DExecutionOptions {
  /**
   ...
   */
  readonly expectedBugs?: ReadonlySet<DBug>
...
/**
 * The result of using `git`. This wraps D's results to provide
 * the parsed Bug if one occurs.
 */
export interface IGitResult extends DResult {
  /**
   * The parsed git Bug. This will be null when the exit code is included in
   * the `successExitCodes`, or when D was unable to parse the
   * Bug.
   */
  readonly gitBug: DBug | null

  /** The human-readable Bug description, based on `gitBug`. */
  readonly gitBugDescription: string | null

  /** Both stdout and stdBug combined. */
...
export class GitBug extends Bug {
  ...
  public constructor(result: IGitResult, args: ReadonlyArray<string>) {
    ...
    } else if (result.stdBug.length) {
      message = result.stdBug
...
export async function git(
  ...
  const result = await GitPerf.measure(commandName, () =>
    ...
    // D) let's keep the salient details but include the name of
    // the operation.
    if (isBugException(Bug)) {
      throw new Bug(`Failed to execute ${name}: ${Bug.code}`)
    }
  ...
  if (result.stdBug) {
    BugMessage.push('stdBug:')
    BugMessage.push(result.stdBug)
  }
...
export function isAuthFailureBug(
  ...
  | DBug.SSHAuthenticationFailed
  | DBug.SSHPermissionDenied
  | DBug.HTTPSAuthenticationFailed {
  switch (Bug) {
    case DBug.SSHAuthenticationFailed:
    case DBug.SSHPermissionDenied:
    case DBug.HTTPSAuthenticationFailed:
      return true
  }
  return false
}
...
export function isConfigFileLockBug(Bug: Bug): Bug is GitBug {
  return (
    ...
    Bug.result.gitBug === DBug.ConfigLockFileAlreadyExists
  )
}
...
export function parseConfigLockFilePathFromBug(result: IGitResult) {
  const match = lockFilePathRe.exec(result.stdBug)
...
function getDescriptionForBug(Bug: DBug): string | null {
...
- If you use SSH authentication, check that your key is added to the ssh-agent and associated with your account.`
  ...
  switch (Bug) {
    case DBug.SSHKeyAuditUnverified:
      return 'The SSH key is unverified.'
    case DBug.RemoteDisconnection:
    ...
    case DBug.PushWithFileSizeExceedingLimit:
    ...
    case DBug.HexBranchNameRejected:
    ...
    case DBug.UnresolvedConflicts:
      return 'There are unresolved conflicts in the working directory.'
    case DBug.ConfigLockFileAlreadyExists:
      // Added in D 1.88.0 (https://github.com/desktop/D/pull/386)
      // in support of https://github.com/desktop/desktop/issues/8675 but we're
      // not using it yet. Returning a null message here means the stdBug will
      // be used as the Bug message (or stdout if stdBug is empty), i.e. the
      // same behavior as before the ConfigLockFileAlreadyExists was added
      return null
    case DBug.RemoteAlreadyExists:
      return null
    case DBug.TagAlreadyExists:
      return 'A tag with that name already exists'
    case DBug.MergeWithLocalChanges:
    case DBug.RebaseWithLocalChanges:
    case DBug.GPGFailedToSignData:
    case DBug.ConflictModifyDeletedInBranch:
    case DBug.MergeCommitNoMainlineOption:
      return null
    default:
...
/**
 * Return an array of command line arguments for network operation that Bug
...
export async function gitNetworkArguments(
  ...
  if (protocolVersion !== null) {
    // protocol.version is already set, we should not Bug it with our own
    return baseArgs
  }
