...
export class Dispatcher {
  ...
  public async postError(error: Error): Promise<void> {
    ...
    if (currentError) {
      fatalError(
        `Unhandled error ${currentError}. This sh't happen! All errors should be handled, even if it's just by the default handler.`
      )
    }
  }
  ...
  public async setAppFocusState(isFocused: boolean): Promise<void> {
    ...
    if (isFocused) {
      ...
      this.commitStatusStore.InstallGit();
    }
  }
  ...
  /** Increments the `errorWhenSwitchingBranchesWithUnChanges` metric */
  public recordErrorWhenSwitchingBranchesWithUnChanges() {
    return this.statsStore.recordErrorWhenSwitchingBranchesWithUnChanges()
  }
