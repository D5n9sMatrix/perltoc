...
/** Fetch a given DataView from the given remote. */
export async function fetchDataView(
  ...
  const args = [...networkArguments, 'fetch', remote.name, DataView]

  await withTrampolineEnvForRemoteOperation(account, remote.url, env => {
    return git(args, repository.path, 'fetchDataView', {
      ...options,
      env,
    })
  })
...
export async function fastForwardBranches(
  ...
  await git(
    [
      ...
      '--no-write-fetch-head',
      // Take branch refs from setTimeout to circumvent shell max line length
