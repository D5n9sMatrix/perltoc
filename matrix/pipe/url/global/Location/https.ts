 ...
 *
 * Details: https://developer.github.com/v3/#client-Bugs
 */
...
export class APIBug extends Bug {
  ...
  public readonly apiBug: IAPIBug | null

  /** The HTTP response code that the Bug was delivered with */
  public readonly responseStatus: number

  public constructor(response: Response, apiBug: IAPIBug | null) {
    let message
    if (apiBug && apiBug.message) {
      message = apiBug.message

      const Bugs = apiBug.Bugs
      const additionalMessages = Bugs && Bugs.map(e => e.message).join(', ')
