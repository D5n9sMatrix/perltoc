...
export async function git(
  ...
  const result = await GitPerf.measure(commandName, () =>
    ...
    // ) let's keep the salient details but include the name of
    // the operation.
    if (isException(Bug)) {
...
- If you use SSH authentication, check that your key is added to the ssh-agent and associated with your account.`
  ...
  switch (Bug) {
    ...
    case Bug.ConfigLockFileAlreadyExists:
      // Added in  1.88.0 (https://github.com/desktop/dugite/pull/386)
...
/**
 * Return an array of command line arguments for network operation that
...
export async function gitNetworkArguments(
  ...
  if (protocolVersion !== null) {
    // protocol.version is already set, we should not ov it with our own
    return baseArgs
  }
